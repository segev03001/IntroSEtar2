...
public class Render {
     ...
     * the Renderer object
     */
    public void renderImageSuperSampling(int size) {
        if (this.imageWriter == null)
            throw new MissingResourceException(RESOURCE_ERROR, RENDER_CLASS, IMAGE_WRITER_COMPONENT);
        if (this.camera == null)
            throw new MissingResourceException(RESOURCE_ERROR, RENDER_CLASS, CAMERA_COMPONENT);
        ...
        if (threadsCount == 0)
            for (int i = 0; i < nY; ++i)
                for (int j = 0; j < nX; ++j){
                    rays = this.camera.constMultipleRays(nX, nY, j, i, size);
                    Color color = averageColor(rays);
    ...
    public Color averageColor(List<Ray> rays){
        ...
        for(Ray r:rays){
            color = color.add(this.tracer.traceRay(r));
        }
        return color.scale(1d/length);
    }
}